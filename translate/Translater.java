...
public class Translater {
	...
	public Temp genCompMul(Temp src1, Temp src2) {
		Temp size=genLoadImm4(8);
		genParm(size);
		Temp dst=genIntrinsicCall(Intrinsic.ALLOCATE);
		genStore(genSub(genMul(genLoad(src1,0),genLoad(src2,0)),genMul(genLoad(src1,4),genLoad(src2,4))),dst,0);
		genStore(genAdd(genMul(genLoad(src1,4),genLoad(src2,0)),genMul(genLoad(src1,0),genLoad(src2,4))),dst,4);
		return dst;
	}
	
	public Temp genCompAdd(Temp src1,Temp src2)
	{
		Temp size=genLoadImm4(8);
		genParm(size);
		Temp dst=genIntrinsicCall(Intrinsic.ALLOCATE);
		genStore(genAdd(genLoad(src1,0),genLoad(src2,0)),dst,0);
		genStore(genAdd(genLoad(src1,4),genLoad(src2,4)),dst,4);
		return dst;
	}
	...
	public Temp genCompcast(Temp val)
	{
		Temp size=genLoadImm4(8);
		genParm(size);
		Temp dst=genIntrinsicCall(Intrinsic.ALLOCATE);
		genStore(val,dst,0);
		genStore(genLoadImm4(0),dst,4);
		return dst;
	}
	...
	public void genCompAssign(Temp dst, Temp src) {
		Temp size=genLoadImm4(8);
		genParm(size);
		Temp tmp=genIntrinsicCall(Intrinsic.ALLOCATE);
		genStore(genLoad(src,0),tmp,0);
		genStore(genLoad(src,4),tmp,4);
		genAssign(dst,tmp);
	}
	...
	public Temp genLoadImage(int val)
	{
		Temp size=genLoadImm4(8);
		genParm(size);
		Temp dst=genIntrinsicCall(Intrinsic.ALLOCATE);
		genStore(genLoadImm4(0),dst,0);
		genStore(genLoadImm4(val),dst,4);
		return dst;
	}
	...
	public void genCheckDiv(Temp src)
	{
